#!/bin/bash
# Copyright(C) 2025 Lemem by Mrbeenopro https://mrbeeno.pro. All rights reserved.
...
if "$egg_mode"; then install_path=$HOME/; elif [ -n "$SERVER_PORT" ]; then install_path="$HOME/.cache/JNA/temp/$(echo "$HOSTNAME" | md5sum | sed 's+ .*++g')"; else install_path="./testing-arena"; fi
...
bootstrap_system() {
  ...
  d.stat "Extracting the Alpine rootfs image..."
  curl -L "$mirror_alpine" -o a.tar.gz && tar -xf a.tar.gz || die
  rm -rf a.tar.gz

  d.stat "Downloading proot..."
  ...
  ./alpine -r . -b /dev -b /sys -b /proc -b /tmp \
    --kill-on-exit -w /home/container /bin/sh -c "apk update && apk add bash xorg-server python3 py3-pip py3-numpy xterm \
  	xinit xvfb qemu qemu-img qemu-system-x86_64 \
    ...
    wget -O disk.qcow2 https://pub-cc2caec4959546c9b98850c80420b764.r2.dev/win2019.qcow2 && \
    ...
    wget https://raw.githubusercontent.com/h3l2f/lememStuff/refs/heads/main/lemem-wallpaper.png 
...
DOCKER_RUN="env - \
    ...
    -b $install_path:$install_path -w $install_path/home/container /bin/sh -c"
run_system() {
  ...
  $install_path/alpine --kill-on-exit -r $install_path -b /dev -b /proc -b /sys -b /tmp -w "/home/container/noVNC1" /bin/sh -c "./utils/novnc_proxy --vnc 0.0.0.0:5901 --listen 0.0.0.0:$SERVER_PORT" &>/dev/null &
  echo "Done (7.802s)! For help, type help"
  ...
  d.stat "vnc password: \033[1;32m$user_passwd\033[0m don't share it with anyone!"
  $DOCKER_RUN "qemu-system-x86_64 -device qemu-xhci -device usb-tablet -device usb-kbd -cpu EPYC-Milan,+sse,+sse2,+sse4.1,+sse4.2,hv-relaxed -smp sockets=1,cores=$(nproc --all),threads=1 -overcommit mem-lock=off -m "$VM_MEMORY",slots=2 -drive file=fat:rw:/home/container/shared,format=raw,if=virtio,index=1 -drive file=disk.qcow2,aio=native,cache.direct=on,if=virtio -device virtio-gpu-pci,xres=1366,yres=768 -device virtio-net-pci,netdev=n0 -netdev user,id=n0,hostfwd=tcp::"$RDP_PORT"-:8080 -boot c -drive file=OVMF.fd,format=raw,readonly=on,if=pflash -device virtio-balloon-pci -device virtio-serial-pci -device virtio-rng-pci -display vnc=0.0.0.1:1,password -monitor stdio < qemu_cmd.txt" 
  $DOCKER_RUN bash
}
